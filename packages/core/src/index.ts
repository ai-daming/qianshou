...
export const conversationRoleSchema = z.enum(['DISCUSSION', 'IMPLEMENTATION', 'REVIEW', 'REPAIR', 'INTEGRATION']);

export type Phase = z.infer<typeof phaseSchema>;
export type Engine = z.infer<typeof engineSchema>;
export type ConversationRole = z.infer<typeof conversationRoleSchema>;
export type Tone = 'neutral' | 'active' | 'success' | 'warning' | 'danger';
export type SlotStatus = 'LOCKED' | 'PAUSED' | 'READY' | 'ACTIVE' | 'DONE' | 'APPROVED' | 'CHANGES_REQUESTED';
...
export type AgentRunEventKind = 'SYSTEM' | 'THOUGHT' | 'TOOL' | 'TOOL_RESULT' | 'MESSAGE' | 'ERROR' | 'RESULT';
...
export const projectConfigSchema: z.ZodType<ProjectConfig> = z.object({
  ...
  integration: z.object({ branch: z.string().min(1), worktree: z.string().startsWith('/'), baseBranch: z.string().min(1) }).strict(),
...
}).refine((value) => Object.values(value).some((item) => item !== undefined), 'no supported control fields');
...
export interface IssueView extends IssueDescriptor {
  ...
  nextAction: { label: string; detail: string; command: string[] | null; shellCommand: string | null };
}

export interface StatusSnapshot {
  project: { id: string; name: string; subtitle: string };
  source: { github: string; git: string; collectedAt: string; stale: boolean; warning: string | null };
  repository: { nameWithOwner: string; url: string; defaultBranch: string };
  milestone: { number: number; title: string; state: string; open_issues: number; closed_issues: number } | null;
...
export function initialIssueControl(project: ProjectConfig, issueNumber: number, now = new Date().toISOString()): IssueControl {
  return {
    issueNumber,
    phase: 'PLANNED',
    candidateSha: null,
...
export function dependencyState(githubIssue: Pick<GithubIssue, 'blockedBy'>) {
  const blockers = githubIssue.blockedBy.filter((issue) => issue.state !== 'CLOSED').map((issue) => issue.number);
  return { ready: blockers.length === 0, blockers };
}

export function issueKindFromGithub(githubIssue: Pick<GithubIssue, 'labels'>): IssueKind {
  return githubIssue.labels.some((label) => label.name.toLowerCase() === 'type:milestone-control') ? 'control' : 'delivery';
}

export function deriveSlots(control: IssueControl, dependenciesReady = true, deliveryPaused = false): IssueView['slots'] {
  ...
  const candidateReady = Boolean(control.candidateSha) && [
    'CANDIDATE_READY', 'REVIEWING', 'CHANGES_REQUESTED', 'APPROVED', 'MERGED_TO_MILESTONE', 'VERIFIED'
  ].includes(control.phase);
  const developerStatus: SlotStatus = ['IMPLEMENTING', 'CHANGES_REQUESTED'].includes(control.phase)
      ? 'ACTIVE'
      : ['CANDIDATE_READY', 'REVIEWING', 'APPROVED', 'MERGED_TO_MILESTONE', 'VERIFIED'].includes(control.phase)
        ? 'DONE'
        : 'READY';
...
export function deriveIssueState(input: {
  ...
  if (projectIssue.kind === 'control') return { key: 'CONTROL', label: '总控', tone: 'neutral', blockers: [] };
  if (openStopConditionCount > 0) return { key: 'NEEDS_DISCUSSION', label: '需要讨论', tone: 'danger', blockers: [] };
  if (control.phase === 'BLOCKED') return { key: 'BLOCKED', label: PHASE_LABELS.BLOCKED, tone: 'warning', blockers: [] };
  if (control.phase === 'NEEDS_HUMAN') return { key: 'NEEDS_HUMAN', label: PHASE_LABELS.NEEDS_HUMAN, tone: 'danger', blockers: [] };
  if (control.phase !== 'PLANNED') return { key: control.phase, label: PHASE_LABELS[control.phase], tone: phaseTone(control.phase), blockers: [] };
  if (githubIssue.state === 'CLOSED') return { key: 'CLOSED', label: 'GitHub 已关闭', tone: 'success', blockers: [] };
...
export function nextAction(input: {
  ...
  const { project, projectIssue, githubIssue, control, worktree, openStopConditionCount = 0 } = input;
  const issueRef = `#${projectIssue.number}`;
  if (!githubIssue) return { label: '刷新 GitHub Milestone', detail: `${issueRef} 不在本次 GitHub Milestone 快照中`, command: null, shellCommand: null };
  if (openStopConditionCount > 0) return {
    label: '回到 Discussion',
  ...
  switch (control.phase) {
    case 'PLANNED': {
      ...
      return { label: '准备 Issue worktree', detail: `使用 Git 从最新 ${project.integration.branch} 创建`, command: null, shellCommand: null };
    }
    case 'WORKTREE_READY': return { label: '启动开发会话', detail: `把冻结任务简报交给 ${worktree?.path ?? 'Issue worktree'} 中的新开发会话`, command: null, shellCommand: null };
    case 'IMPLEMENTING': return { label: '等待候选提交', detail: '开发者必须返回测试证据和 candidate SHA', command: null, shellCommand: null };
    case 'CANDIDATE_READY': return { label: '派发独立 Review', detail: `Reviewer 只能审 ${control.candidateSha ?? '尚未填写的 candidate SHA'}`, command: null, shellCommand: null };
    case 'REVIEWING': return { label: '等待 Review verdict', detail: 'Reviewer 不得直接修改候选代码', command: null, shellCommand: null };
    case 'CHANGES_REQUESTED': return { label: '返给原开发者修复', detail: '修复后必须生成新的 candidate SHA', command: null, shellCommand: null };
    case 'APPROVED': return { label: '执行集成门禁', detail: `确认 Review、测试和 ${control.candidateSha ?? 'candidate'} 后再请求人工合并`, command: null, shellCommand: null };
    case 'MERGED_TO_MILESTONE': return { label: '验证集成分支', detail: '确认合入 SHA、回归测试和后继 Issue 依赖', command: null, shellCommand: null };
    case 'VERIFIED': return { label: '工作包已完成', detail: '选择下一个依赖已满足的 Issue', command: null, shellCommand: null };
    default: return { label: '等待人工处理', detail: control.note || PHASE_LABELS[control.phase], command: null, shellCommand: null };
  }
}

export function shellCommand(args: string[] | null): string | null {
  if (!args) return null;
  return args.map((value) => /^[A-Za-z0-9_./:@-]+$/.test(value) ? value : `'${value.replaceAll("'", "'\\''")}'`).join(' ');
}
...
export function buildAgentInputPackage(input: {
  ...
  const { role } = input;
  if (role !== 'DISCUSSION' && !input.brief) throw new Error(`${role} requires a frozen task brief`);
  ...
  const roleInstruction: Record<ConversationRole, string> = {
    DISCUSSION: 'Help the human clarify the problem, decisions, acceptance criteria, non-goals, constraints, and unresolved questions. Do not edit files.',
    IMPLEMENTATION: 'Implement the frozen task brief in the assigned worktree. Use repository instructions and TDD. Do not merge, push, deploy, or broaden scope.',
    REVIEW: 'Independently review only the frozen candidate SHA. Do not edit or repair code. Return findings with evidence and a verdict.',
    REPAIR: 'Repair accepted Review findings in the original worktree, rerun tests, and produce a new candidate for independent re-review.',
    INTEGRATION: 'Verify the approved candidate and integration evidence. Do not merge or push without explicit human authority.'
  };
  const lines = [
  ...
  if (input.brief) lines.push('', `Task brief: ${input.brief.id} v${input.brief.version}`, '', input.brief.content);
  if (input.workspace) lines.push('', `Base SHA: ${input.workspace.baseSha}`, `Branch: ${input.workspace.branch}`, `Worktree: ${input.workspace.path}`);
  ...
  lines.push('', 'Treat Git, tests, and repository state as evidence. Do not treat another Agent completion claim as proof.');
...
export function buildTaskContract(input: {
  ...
  if (role === 'reviewer') return [
    '# Qianshou Candidate Reviewer Contract', '', ...common, '',
  ...
  return [
    '# Qianshou Issue Implementer Contract', '', ...common, '',
