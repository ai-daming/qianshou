...
describe('delivery domain', () => {
  it('keeps a planned Issue blocked until the GitHub dependency closes', () => {
    ...
    expect(deriveIssueState({ projectIssue, githubIssue, control })).toMatchObject({ key: 'WAITING_DEPENDENCY', blockers: [224] });
  ...
  it('locks every new delivery slot while a GitHub dependency remains open', () => {
    ...
    expect(deriveSlots({ ...control, phase: 'CANDIDATE_READY', candidateSha: 'abcdef1' }, false)).toEqual({
      developerStatus: 'LOCKED',
      reviewerStatus: 'LOCKED'
    });
  });
  ...
  it('keeps manual phase distinct from GitHub state', () => {
    expect(deriveIssueState({ projectIssue, githubIssue, control: { ...control, phase: 'REVIEWING' } }).key).toBe('REVIEWING');
  });
  ...
  it('pauses delivery for discussion without replacing the preserved delivery phase', () => {
    ...
    expect(deriveIssueState({ projectIssue, githubIssue, control: reviewing, openStopConditionCount: 1 })).toMatchObject({
      key: 'NEEDS_DISCUSSION',
      label: '需要讨论'
    });
    expect(reviewing.phase).toBe('REVIEWING');
    expect(deriveSlots(reviewing, true, true)).toEqual({ developerStatus: 'PAUSED', reviewerStatus: 'PAUSED' });
    expect(nextAction({ project, projectIssue, githubIssue, control: reviewing, openStopConditionCount: 1 })).toMatchObject({
      label: '回到 Discussion',
      command: null
    });
  });
  ...
  it('accepts only connection/runtime configuration and rejects duplicated GitHub facts', () => {
    ...
    expect(projectConfigSchema.safeParse({ ...minimalConfig, issues: [{ number: 19, kind: 'delivery' }] }).success).toBe(false);
    expect(projectConfigSchema.safeParse({ ...minimalConfig, name: 'Demo', subtitle: 'M7' }).success).toBe(false);
    expect(projectConfigSchema.safeParse({ ...minimalConfig, milestone: { number: 7, title: 'Demo M7' } }).success).toBe(false);
  });

  it('derives the Control Issue role from the native GitHub label', () => {
    expect(issueKindFromGithub({ labels: [{ name: 'type:milestone-control', color: '000000' }] })).toBe('control');
    expect(issueKindFromGithub({ labels: [{ name: 'enhancement', color: '000000' }] })).toBe('delivery');
  });

  it('locks Reviewer until a candidate is frozen', () => {
    expect(deriveSlots({ ...control, phase: 'WORKTREE_READY' }).reviewerStatus).toBe('LOCKED');
    expect(deriveSlots({ ...control, phase: 'CANDIDATE_READY', candidateSha: 'abcdef1' }).reviewerStatus).toBe('READY');
  });

  it('validates control payloads at the boundary', () => {
    expect(controlPatchSchema.parse({ phase: 'CANDIDATE_READY', candidateSha: 'abcdef1', reviewerEngine: 'claude' })).toEqual({ phase: 'CANDIDATE_READY', candidateSha: 'abcdef1', reviewerEngine: 'claude' });
  ...
  it('validates structured Stop Conditions and explicit resolution outcomes', () => {
    expect(createStopConditionSchema.parse({
      category: 'SCOPE_CHANGE',
      summary: '当前 Issue 的范围不足以承载新发现的业务规则',
      detail: '需要讨论拆分 Issue 或提升为新的 Milestone。'
    })).toMatchObject({ category: 'SCOPE_CHANGE' });
    expect(resolveStopConditionSchema.parse({
      resolution: '拆出基础状态机 Issue，当前 Issue 保持等待。',
      outcome: 'SPLIT_ISSUE'
    })).toMatchObject({ outcome: 'SPLIT_ISSUE' });
    expect(() => resolveStopConditionSchema.parse({ resolution: '', outcome: 'CONTINUE' })).toThrow();
  });

  it('quotes unsafe shell arguments', () => {
    expect(shellCommand(['git', 'worktree', 'add', '--detach', "it's-ready"])).toBe("git worktree add --detach 'it'\\''s-ready'");
  });
});
