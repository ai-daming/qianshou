...
import { createQianshouServer, parseGithubDependencySnapshot, type ExternalCollector } from '../src/server.js';
...
describe('Qianshou HTTP boundary', () => {
  it('keeps Reviewer locked until an auditable candidate SHA is frozen', async () => {
    ...
    const collector: ExternalCollector = async (project) => ({
      ...
      repository: { nameWithOwner: 'owner/repo', url: 'https://github.com/owner/repo', defaultBranch: 'main' },
      ...
      run: async (input: AgentExecutionInput): Promise<AgentExecutionResult> => {
        ...
        return {
          ...
          text: input.prompt.includes('# 开发说明生成任务')
            ? '# 开发说明\n\n## 已确认决策\n\n上户必须人工确认。\n\n## 未决问题\n\n无。'
            : input.role === 'DISCUSSION' ? '讨论结论：上户必须人工确认。' : `${input.role} 已收到交接包`,
          rawEvents: []
        };
      }
    };
    ...
    expect(initialPayload.issues.map((issue: { number: number }) => issue.number)).toEqual([224, 19, 149]);
    ...
    const allowedDiscussionMessage = await fetch(`${baseUrl}/api/conversations/${blockedDiscussionPayload.id}/messages`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    const blockedDevelopmentBrief = await fetch(`${baseUrl}/api/conversations/${blockedDiscussionPayload.id}/development-brief`, {
      method: 'POST'
    });
    ...
    const blockedBriefFreeze = await fetch(`${baseUrl}/api/conversations/${blockedDiscussionPayload.id}/briefs`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    const generateBrief = await fetch(`${baseUrl}/api/conversations/${discussion.id}/development-brief`, { method: 'POST' });
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const response = await fetch(`${baseUrl}/api/issues/224/collaboration`);
      collaboration = await response.json();
      if (collaboration.turns.find((turn: { id: string }) => turn.id === generationTurn.id)?.status === 'COMPLETED') break;
      await new Promise((resolve) => setTimeout(resolve, 10));
    }
    const generatedMessage = collaboration.conversations[0].messages.find((message: { turnId: string; role: string }) => message.turnId === generationTurn.id && message.role === 'AGENT');
    expect(generatedMessage.text).toContain('# 开发说明');
    expect(agentInputs.at(-1)?.prompt).toContain('仅整理讨论中已经明确确认的结论');

    const continueDiscussion = await fetch(`${baseUrl}/api/conversations/${discussion.id}/messages`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    expect(stop).toMatchObject({ originPhase: 'REVIEWING', candidateSha: 'abcdef1', status: 'OPEN' });
    ...
    const continueReviewWhilePaused = await fetch(`${baseUrl}/api/conversations/${review.id}/messages`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  it('parses native blockedBy relationships returned by GitHub GraphQL', () => {
    const dependencies = parseGithubDependencySnapshot(JSON.stringify({
      data: {
        repository: {
          issue19: {
            number: 19,
  ...
  it('exposes live Agent progress and cancels the process owned by a running turn', async () => {
    ...
    const collector: ExternalCollector = async (project) => {
      collectorCalls += 1;
      return ({
      source: { github: 'fake', git: 'fake', collectedAt: new Date().toISOString(), stale: false, warning: null },
      repository: { nameWithOwner: 'owner/repo', url: 'https://github.com/owner/repo', defaultBranch: 'main' },
      milestone: { number: 7, title: 'Demo M7', state: 'open', open_issues: 1, closed_issues: 0 },
      githubIssues: [{
        number: 224, title: 'Lifecycle', body: '', state: 'OPEN',
        url: 'https://github.com/owner/repo/issues/224', updatedAt: new Date().toISOString(),
        milestone: { title: 'Demo M7' }, labels: [], assignees: [], blockedBy: []
      }],
      pulls: [], worktrees: [],
      integration: {
        ...project.integration, sha: '7654321', dirty: [], statusLine: 'milestone-7',
        aheadMain: 0, behindMain: 0, hasRemoteBranch: true
      }
      });
    };
    ...
    const agentRunner = {
      run: (input: AgentExecutionInput) => {
        ...
        return new Promise<AgentExecutionResult>((_resolve, reject) => { rejectRun = reject; });
      },
      cancel: (runId: string) => {
    ...
    const server = await createQianshouServer({ configPath, statePath, collector, agentRunner, refreshIntervalMs: 0 });
    servers.push(server);
    await new Promise<void>((resolve) => server.listen(0, '127.0.0.1', resolve));
    const address = server.address() as AddressInfo;
    const baseUrl = `http://127.0.0.1:${address.port}`;

    const discussion = await (await fetch(`${baseUrl}/api/issues/224/conversations`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ role: 'DISCUSSION', engine: 'claude' })
    })).json();
    const turn = await (await fetch(`${baseUrl}/api/conversations/${discussion.id}/messages`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
    ...
    expect(stopped.turns[0]).toMatchObject({ status: 'CANCELLED', progress: { summary: '运行已停止' } });
  ...
  it('serves the last verified snapshot when gh exits without network diagnostics', async () => {
    ...
    const collector: ExternalCollector = async (project) => {
      calls += 1;
      if (calls > 1) {
        throw new Error('Command failed: gh api repos/owner/repo/pulls?state=all&sort=updated&direction=desc&per_page=100');
      }
      return {
        source: { github: 'gh CLI', git: 'git CLI', collectedAt: '2026-08-14T00:00:00.000Z', stale: false, warning: null },
        repository: { nameWithOwner: 'owner/repo', url: 'https://github.com/owner/repo', defaultBranch: 'main' },
        milestone: { number: 7, title: 'Demo M7', state: 'open', open_issues: 1, closed_issues: 0 },
        githubIssues: [{
          number: 224,
    ...
    const server = await createQianshouServer({ configPath, statePath, collector, refreshIntervalMs: 0 });
    servers.push(server);
    await new Promise<void>((resolve) => server.listen(0, '127.0.0.1', resolve));
    const address = server.address() as AddressInfo;
    const baseUrl = `http://127.0.0.1:${address.port}`;
