...
describe('Git-native Issue worktree creation', () => {
  it('creates an Issue branch and worktree from the configured Milestone integration branch', async () => {
    ...
    try {
      ...
      await git(repository, 'worktree', 'add', '-b', 'codex/milestone-7-poster-engine-baseline', integrationWorktree, 'main');
      ...
      expect(await git(repository, 'worktree', 'list', '--porcelain')).toContain(`worktree ${workspace.path}`);
    } finally {
