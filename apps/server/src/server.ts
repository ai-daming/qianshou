...
interface RawRepository { full_name: string; html_url: string; default_branch: string }
interface RawMilestone { number: number; title: string; state: string; open_issues: number; closed_issues: number }
interface RawIssue {
  number: number; title: string; body: string | null; state: string; html_url: string; updated_at: string;
  pull_request?: unknown; milestone: { title: string } | null;
  labels: Array<{ name: string; color: string }>;
  assignees: Array<{ login: string }>;
}
interface RawPull {
  number: number; title: string; body: string | null; state: string; html_url: string; updated_at: string;
  merged_at: string | null; draft: boolean; base: { ref: string } | null; head: { ref: string } | null;
}
...
export type WorktreeCreator = (project: ProjectConfig, issueNumber: number) => Promise<IssueWorkspaceBinding>;
...
function isTransientNetworkError(error: unknown) {
  ...
  if (/\bEOF\b|socket hang up|ECONNRESET|ETIMEDOUT|TLS|network|GraphQL request failed/i.test(message)) return true;
...
async function runCommand(file: string, args: string[], cwd: string, options: { retries?: number; timeout?: number } = {}) {
...
export function parseGithubDependencySnapshot(value: string): Map<number, GithubIssue['blockedBy']> {
  const payload = parseJson<RawDependencySnapshot>(value);
  if (payload.errors?.length) {
    throw new Error(`GitHub dependency query failed: ${payload.errors.map((error) => error.message ?? 'unknown error').join('; ')}`);
  }
  if (!payload.data?.repository) throw new Error('GitHub dependency query returned no repository data');
  const result = new Map<number, GithubIssue['blockedBy']>();
  for (const issue of Object.values(payload.data.repository)) {
    if (!issue) continue;
    result.set(issue.number, issue.blockedBy.nodes.map(({ number, state }) => ({ number, state: state.toUpperCase() })));
  }
  return result;
}
...
function githubDependencyQuery(project: ProjectConfig, issueNumbers: number[]) {
  ...
  if (!owner || !name || extra.length) throw new Error(`invalid GitHub repository slug: ${project.repository.slug}`);
...
function issuePulls(issueNumber: number, pulls: PullRequest[]) {
  ...
  return pulls.filter((pull) => pattern.test(`${pull.title}\n${pull.body ?? ''}\n${pull.headRefName ?? ''}`));
}
...
export const createGitWorktree: WorktreeCreator = async (project, issueNumber) => {
  ...
  const baseSha = await runCommand('git', ['rev-parse', project.integration.branch], project.integration.worktree);
  ...
  ], project.repository.path, { timeout: 120_000 });
...
export const collectExternalStatus: ExternalCollector = async (project) => {
  ...
  const [repoRaw, milestonesRaw, issuesRaw, pullsRaw, gitWorktreesRaw, branchRaw, divergenceRaw, remoteBranchRaw] = await Promise.all([
    runCommand('gh', ['api', repositoryApi], project.repository.path),
    runCommand('gh', ['api', `${repositoryApi}/milestones?state=all&per_page=100`], project.repository.path),
    runCommand('gh', ['api', `${repositoryApi}/issues?milestone=${project.milestone.number}&state=all&per_page=100`], project.repository.path),
    runCommand('gh', ['api', `${repositoryApi}/pulls?state=all&sort=updated&direction=desc&per_page=100`], project.repository.path),
    runCommand('git', ['worktree', 'list', '--porcelain'], project.repository.path),
    runCommand('git', ['status', '--short', '--branch'], project.integration.worktree),
    runCommand('git', ['rev-list', '--left-right', '--count', `origin/${project.integration.baseBranch}...${project.integration.branch}`], project.integration.worktree),
    runCommand('git', ['branch', '-r', '--list', `origin/${project.integration.branch}`], project.integration.worktree)
  ]);
  ...
  const dependencySnapshot = issueNumbers.length
    ? parseGithubDependencySnapshot(await runCommand('gh', ['api', 'graphql', '-f', `query=${githubDependencyQuery(project, issueNumbers)}`], project.repository.path))
    : new Map<number, GithubIssue['blockedBy']>();
  for (const issueNumber of issueNumbers) {
    if (!dependencySnapshot.has(issueNumber)) throw new Error(`GitHub dependency snapshot missing Issue #${issueNumber}`);
  }
  const githubIssues = rawIssues.map((issue) => normalizeIssue(issue, dependencySnapshot.get(issue.number) ?? []));
  ...
  return {
    source: { github: 'gh CLI → GitHub REST + GraphQL APIs', git: 'git CLI', collectedAt: new Date().toISOString(), stale: false, warning: null },
    repository: { nameWithOwner: repository.full_name, url: repository.html_url, defaultBranch: repository.default_branch },
    milestone,
    githubIssues,
    pulls,
    worktrees,
...
function redactError(error: unknown) {
  return errorMessage(error).replace(/ghp_[A-Za-z0-9_]+/g, '[redacted]').replace(/github_pat_[A-Za-z0-9_]+/g, '[redacted]');
}
...
function projectView(project: ProjectConfig) {
  return { id: project.id, name: `${project.repository.slug} · M${project.milestone.number}`, subtitle: 'GitHub Milestone' };
}

export async function createQianshouServer(options: {
  configPath?: string;
  statePath?: string;
  ...
  const getProject = (id: string | null) => projects.find((project) => project.id === id) ?? projects[0];
  ...
  async function getExternal(project: ProjectConfig) {
    ...
    if (cached && Date.now() - new Date(cached.source.collectedAt).getTime() < refreshIntervalMs) return cached;
    if (!inflight.has(project.id)) {
      const current = collector(project).then((result) => {
        cache.set(project.id, result);
        return result;
      }).catch((error: unknown) => {
        const previous = cache.get(project.id);
        if (!previous || !isTransientNetworkError(error)) throw error;
        return { ...previous, source: { ...previous.source, stale: true, warning: `GitHub 暂时断线，显示最近快照：${redactError(error)}` } };
  ...
  async function buildStatus(project: ProjectConfig): Promise<StatusSnapshot> {
    ...
    const state = await store.read(project, external.githubIssues.map((issue) => issue.number));
    ...
    const issues = external.githubIssues.map((githubIssue) => {
      ...
      const openStopConditionCount = Object.values(projectState.stopConditions)
        .filter((item) => item.issueNumber === projectIssue.number && item.status === 'OPEN').length;
      return {
        ...projectIssue,
        github: githubIssue,
        pulls: issuePulls(projectIssue.number, external.pulls).map(({ body: _body, ...pull }) => pull),
        worktrees,
        control,
        ...
        nextAction: nextAction({ project, projectIssue, githubIssue, control, openStopConditionCount, ...(worktree ? { worktree } : {}) })
      };
    });
    return {
      project: {
    ...
    try {
      ...
      if (request.method === 'POST' && createConversationMatch) {
        ...
        if (input.role !== 'DISCUSSION' && issue.attention.required) {
          sendJson(response, 409, { error: 'delivery_paused', message: 'Resolve the open Stop Conditions in Discussion before starting another delivery conversation.' });
          return;
        }
      ...
      const developmentBriefMatch = url.pathname.match(/^\/api\/conversations\/([^/]+)\/development-brief$/);
      if (request.method === 'POST' && (messageMatch || developmentBriefMatch)) {
        const generatingDevelopmentBrief = Boolean(developmentBriefMatch);
        const conversationId = decodeURIComponent((messageMatch ?? developmentBriefMatch)?.[1] ?? '');
        ...
        if (generatingDevelopmentBrief && conversation.role !== 'DISCUSSION') {
          sendJson(response, 409, { error: 'development_brief_requires_discussion', message: '开发说明只能从 Discuss 对话生成。' });
          return;
        }
        if (generatingDevelopmentBrief || conversation.role !== 'DISCUSSION') {
          const status = await buildStatus(project);
        ...
        if (conversation.role !== 'DISCUSSION' && collaboration.stopConditions.some((item) => item.status === 'OPEN')) {
          sendJson(response, 409, { error: 'delivery_paused', message: 'This delivery conversation is paused by an open Stop Condition. Continue in Discussion first.' });
          return;
        }
        ...
        if (conversation.role === 'IMPLEMENTATION' || conversation.role === 'REPAIR') {
          await store.updateIssue(project, conversation.issueNumber, { phase: 'IMPLEMENTING', note: `${conversation.engine} ${conversation.role} run started` });
        } else if (conversation.role === 'REVIEW') {
          await store.updateIssue(project, conversation.issueNumber, { phase: 'REVIEWING', note: `${conversation.engine} independent Review started` });
        }
        const cwd = conversation.role === 'DISCUSSION'
      ...
      if (request.method === 'POST' && cancelTurnMatch) {
        ...
        if (turn.status !== 'RUNNING') {
          sendJson(response, 409, { error: 'turn_not_running', message: `Agent turn is already ${turn.status}` });
          return;
        }
        const reason = '用户停止了本次 Agent 运行，可以在同一对话中重新发送。';
        const conversation = await store.cancelConversationTurn(project, turn.conversationId, turn.id, reason);
      ...
      if (request.method === 'POST' && workspaceMatch) {
        ...
        if (issue.attention.required) {
          sendJson(response, 409, { error: 'delivery_paused', message: 'Resolve the open Stop Conditions in Discussion before creating a worktree.' });
          return;
        }
        const collaboration = await store.getCollaboration(project, issueNumber);
        if (!collaboration.briefs[0]) {
          sendJson(response, 409, { error: 'brief_required', message: 'Freeze a task brief before creating the Issue workspace.' });
          return;
        }
        if (collaboration.workspace) {
          sendJson(response, 409, { error: 'workspace_exists', message: 'This Issue already has a Qianshou workspace binding.' });
          return;
        }
        ...
        await store.updateIssue(project, issueNumber, { phase: 'WORKTREE_READY', note: `${workspace.branch} at ${workspace.path}` });
        sendJson(response, 201, workspace);
        return;
      }
      const createStopConditionMatch = url.pathname.match(/^\/api\/issues\/(\d+)\/stop-conditions$/);
      ...
      const resolveStopConditionMatch = url.pathname.match(/^\/api\/stop-conditions\/([^/]+)\/resolve$/);
      ...
      if (request.method === 'GET' && contractMatch) {
        ...
        if (role === 'implementer' && issue.slots.developerStatus === 'LOCKED') {
          sendJson(response, 409, { error: 'implementer_locked', message: 'Close the GitHub Blocked by Issues before creating a development conversation.' });
          return;
        }
        if (role === 'reviewer' && issue.slots.reviewerStatus === 'LOCKED') {
          sendJson(response, 409, { error: 'reviewer_locked', message: 'Freeze a candidate SHA before creating a Review conversation.' });
          return;
        }
        const worktree = issue.worktrees[0];
        sendJson(response, 200, {
          role,
          contract: buildTaskContract({ role, project, projectIssue: issue, githubIssue: issue.github, control: issue.control, ...(worktree ? { worktree } : {}) })
        });
        return;
      }
      ...
      const badRequest = error instanceof SyntaxError || /invalid|unsupported|body too large|supported control|candidateSha|Invalid input|too_small|too_big|invalid_type/.test(message);
      sendJson(response, badRequest ? 400 : 502, { error: badRequest ? 'invalid_request' : 'request_failed', message: redactError(error), at: new Date().toISOString() });
    }
  });
}

const isEntrypoint = process.argv[1] ? fileURLToPath(import.meta.url) === resolve(process.argv[1]) : false;
if (isEntrypoint) {
