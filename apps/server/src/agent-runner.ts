...
function scrubValue(value: unknown, depth = 0): unknown {
  ...
  return Object.fromEntries(Object.entries(object).slice(0, 16).map(([key, item]) => [
    key,
...
function progressEvent(engine: Engine, raw: unknown, sequence: number): Omit<AgentRunEvent, 'at'> {
  ...
  const build = (kind: AgentRunEvent['kind'], title: string, detail: unknown = null) => ({
    sequence, kind, title, detail: safePreview(detail)
  });
  if (!event) return build('SYSTEM', `${engine === 'claude' ? 'Claude Code' : 'Codex'} 正在处理`);
  if (engine === 'claude') {
    if (event.type === 'system' && event.subtype === 'init') {
      return build('SYSTEM', 'Claude Code 已连接', { model: event.model, tools: Array.isArray(event.tools) ? event.tools.length : undefined });
    }
    if (event.type === 'result') {
      return build(event.is_error === true ? 'ERROR' : 'RESULT', event.is_error === true ? 'Claude Code 运行失败' : 'Claude Code 完成本轮', {
    ...
    if (event.type === 'assistant') {
      ...
      const text = content.map((item) => stringValue(objectValue(item)?.text)).filter(Boolean).join('\n');
  ...
  if (event.type === 'thread.started') return build('SYSTEM', 'Codex 已连接', { thread_id: event.thread_id });
  ...
  if (event.type === 'item.started' || event.type === 'item.completed') {
    ...
    if (item?.type === 'command_execution') {
      ...
      return build(completed ? 'TOOL_RESULT' : 'TOOL', completed ? '命令执行完成' : '执行命令', completed
        ? { exit_code: item.exit_code, output: item.aggregated_output }
        : item.command);
    }
    if (item?.type === 'mcp_tool_call') {
      return build(event.type === 'item.completed' ? 'TOOL_RESULT' : 'TOOL', `${event.type === 'item.completed' ? '工具调用完成' : '调用工具'} ${stringValue(item.server) ?? ''} ${stringValue(item.tool) ?? ''}`.trim(), event.type === 'item.completed' ? item.result : item.arguments);
    }
    ...
    return build('SYSTEM', `Codex ${event.type === 'item.completed' ? '完成' : '开始'} ${stringValue(item?.type) ?? '事件'}`, item);
  }
...
function parseClaude(events: unknown[]): AgentExecutionResult {
  ...
  for (const raw of events) {
    ...
    if (event.type === 'assistant') {
      const message = objectValue(event.message);
      const content = Array.isArray(message?.content) ? message.content : [];
      const text = content.map((item) => stringValue(objectValue(item)?.text)).filter((item): item is string => Boolean(item)).join('\n');
...
function parseCodex(events: unknown[]): AgentExecutionResult {
  ...
  for (const raw of events) {
    ...
    if (event.type === 'error') throw new Error(stringValue(event.message) ?? 'Codex returned an error event');
  }
  ...
  async run(input: AgentExecutionInput): Promise<AgentExecutionResult> {
    ...
    const output = await new Promise<{ stdout: string; stderr: string }>((resolvePromise, reject) => {
      ...
      child.once('close', (code, signal) => {
        ...
        if (code !== 0) {
          reject(new Error(`${input.engine} Agent exited with ${code ?? signal}: ${stderrText.slice(-4000)}`));
          return;
        }
  ...
  private arguments(input: AgentExecutionInput): string[] {
    if (input.engine === 'claude') {
      return [
        ...
        '--verbose',
        '--permission-mode', writableRoles.includes(input.role) ? 'acceptEdits' : 'plan'
      ];
    }
    if (input.sessionId) {
      return ['exec', 'resume', '--json', input.sessionId, '-'];
    }
    return ['exec', '--json', '--sandbox', writableRoles.includes(input.role) ? 'workspace-write' : 'read-only', '-C', input.cwd, '-'];
  }
}
