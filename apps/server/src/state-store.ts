...
export class JsonStateStore {
  ...
  async read(project: ProjectConfig, observedIssueNumbers: number[] = []): Promise<LedgerState> {
    ...
    for (const issueNumber of observedIssueNumbers) {
      projectState.issues[String(issueNumber)] ??= initialIssueControl(
        project,
        issueNumber,
        this.clock(),
      );
  ...
  async updateIssue(project: ProjectConfig, issueNumber: number, patch: ControlPatch) {
    const operation = this.writeChain.then(async () => {
      ...
      const before =
        projectState.issues[key] ?? initialIssueControl(project, issueNumber, this.clock());
  ...
  async completeConversationTurn(
    ...
    return this.mutate(project, async (state) => {
      ...
      if (!conversation || !turn || turn.conversationId !== conversationId)
      ...
      turn.progress = {
        eventCount: turn.progress?.eventCount ?? 0,
        summary: "运行完成",
        updatedAt: now,
        events: turn.progress?.events ?? [],
      };
  ...
  async updateConversationTurnProgress(
    ...
    progress: {
      eventCount: number;
      summary: string;
      event: Omit<NonNullable<AgentTurn["progress"]>["events"][number], "at">;
    },
  ...
  async recoverOrphanedTurns(project: ProjectConfig): Promise<number> {
    return this.mutate(project, async (state) => {
      const projectState = this.requireProjectState(state, project);
      const orphaned = Object.values(projectState.turns).filter(
      ...
      for (const turn of orphaned) {
        ...
        const now = this.clock();
        const reason =
          "Qianshou 服务已重启，无法恢复上一次 Agent 进程；本次运行已标记为取消，可以重新发送。";
        turn.status = "CANCELLED";
        turn.error = reason;
        turn.completedAt = now;
        turn.progress = {
  ...
  async freezeBrief(
    ...
    return this.mutate(project, async (state) => {
      ...
      const conversation = projectState.conversations[conversationId];
      if (!conversation) throw new Error(`conversation not found: ${conversationId}`);
      if (conversation.role !== "DISCUSSION")
        throw new Error("task briefs can only be frozen from a discussion");
      const version =
        Object.values(projectState.briefs).filter(
          (brief) => brief.issueNumber === conversation.issueNumber,
        ).length + 1;
      const id = this.idFactory();
  ...
  async attachWorkspace(
    project: ProjectConfig,
    workspace: IssueWorkspaceBinding,
  ...
  async createStopCondition(
    project: ProjectConfig,
    issueNumber: number,
    input: CreateStopCondition,
  ): Promise<IssueStopCondition> {
    return this.mutate(project, async (state) => {
      const projectState = this.requireProjectState(state, project);
  ...
  async resolveStopCondition(
    project: ProjectConfig,
    stopConditionId: string,
    input: ResolveStopCondition,
  ): Promise<IssueStopCondition> {
    return this.mutate(project, async (state) => {
      const projectState = this.requireProjectState(state, project);
      ...
      if (stopCondition.status === "RESOLVED")
        throw new Error("Stop Condition is already resolved");
  ...
  async getCollaboration(project: ProjectConfig, issueNumber: number): Promise<CollaborationView> {
    ...
    return {
      issueNumber,
      conversations: Object.values(projectState.conversations)
        .filter((item) => item.issueNumber === issueNumber)
        .sort((left, right) => left.createdAt.localeCompare(right.createdAt)),
      turns: Object.values(projectState.turns)
        .filter((item) => item.issueNumber === issueNumber)
        .sort((left, right) => right.startedAt.localeCompare(left.startedAt)),
      briefs: Object.values(projectState.briefs)
        .filter((item) => item.issueNumber === issueNumber)
  ...
  async findStopCondition(
    project: ProjectConfig,
    stopConditionId: string,
  ): Promise<IssueStopCondition | null> {
    const state = await this.read(project);
  ...
  private async mutate<T>(
    project: ProjectConfig,
    action: (state: LedgerState) => Promise<T>,
  ): Promise<T> {
    const operation = this.writeChain.then(async () => {
      const state = await this.read(project);
