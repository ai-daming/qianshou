...
const formatTime = (value: string | null | undefined) => value ? new Intl.DateTimeFormat('zh-CN', {
...
function selectedIssue(): IssueView | null {
  return snapshot?.issues.find((issue) => issue.number === selectedIssueNumber) ?? snapshot?.issues[0] ?? null;
}
...
function renderOverview() {
  ...
  const next = current.issues.find((issue) => ['READY', 'WORKTREE_READY', 'IMPLEMENTING', 'CANDIDATE_READY', 'REVIEWING', 'CHANGES_REQUESTED', 'APPROVED'].includes(issue.state.key));
  element('handoff-title').textContent = next ? `#${next.number} · ${next.state.label}` : '当前没有可推进工作包';
  element('handoff-detail').textContent = next?.nextAction.detail ?? '检查依赖、GitHub 状态和本地账本。';
  const needsYou = current.issues.filter((issue) => ['NEEDS_DISCUSSION', 'NEEDS_HUMAN', 'APPROVED'].includes(issue.state.key)).length;
  element('needs-you-count').textContent = String(needsYou).padStart(2, '0');
  element('milestone-count').textContent = current.milestone ? `${current.milestone.open_issues}/${current.milestone.closed_issues}` : 'N/A';
  element('milestone-state').textContent = current.milestone ? `${current.milestone.state.toUpperCase()} · OPEN/CLOSED` : 'Milestone 未找到';
  element('integration-sha').textContent = shortSha(current.integration.sha);
  element('integration-state').textContent = `${current.integration.aheadMain} ahead · ${current.integration.behindMain} behind · ${current.integration.dirty.length ? `${current.integration.dirty.length} dirty` : 'clean'}`;
  ...
  if (current.source.warning || !current.integration.hasRemoteBranch) {
    strip.textContent = current.source.warning ?? `注意：集成分支 ${current.integration.branch} 尚无远程同名分支。`;
...
function renderIssueList() {
  ...
  element('issue-list').innerHTML = current.issues.map((issue, index) => {
    const dependencies = issue.github ? dependencyState(issue.github) : { ready: false, blockers: [] };
    const blockerText = dependencies.blockers.map((number) => `#${number}`).join('、') || 'GitHub 状态未知';
    return `
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-issue]').forEach((button) => button.addEventListener('click', () => {
...
function renderSelectedIssue() {
  ...
  element('issue-description').textContent = issue.github?.body.split('\n').find((line) => line.trim())?.slice(0, 180) ?? '查看外部事实、讨论与 Agent 接力。';
  ...
  element('github-meta').textContent = issue.github ? `更新 ${formatTime(issue.github.updatedAt)}` : '项目配置与 GitHub 不一致';
  ...
  element('worktree-meta').textContent = boundWorkspace
    ...
    : detectedWorktree ? `${detectedWorktree.branch ?? 'detached'} · ${shortSha(detectedWorktree.sha)}` : '尚未识别 Issue worktree';
  ...
  element('review-meta').textContent = issue.control.candidateSha ? `审 ${shortSha(issue.control.candidateSha)}` : '候选未冻结';
  ...
  element('pull-list').innerHTML = issue.pulls.length
    ? issue.pulls.map((pull) => `<div class="relation-row"><a href="${escapeHtml(pull.url)}" target="_blank" rel="noreferrer">PR #${pull.number} · ${escapeHtml(pull.title)}</a><code>${escapeHtml(pull.state)}</code></div>`).join('')
...
function renderCollaboration() {
  ...
  const developmentBriefTurn = collaboration?.turns.find((turn) => turn.conversationId === discussion?.id && turn.intent === 'DEVELOPMENT_BRIEF') ?? null;
  const generatedDevelopmentBrief = developmentBriefTurn?.status === 'COMPLETED'
    ? discussion?.messages.find((message) => message.turnId === developmentBriefTurn.id && message.role === 'AGENT')?.text ?? null
    : null;
  ...
  if (paused) {
    ...
    element('next-action-detail').textContent = `${blockerText} 尚未关闭。当前只能继续 Discussion 和查看已有记录，不能产生或修改任何交付内容。`;
  } else if (!brief) {
    ...
    element('next-action-detail').textContent = '使用 Git 从 Milestone 集成分支创建 Issue 分支和 worktree。';
  } else if (!implementation) {
    element('next-action-title').textContent = '开始编码';
    element('next-action-detail').textContent = '选择引擎，新的开发会话将收到已确认的开发说明和 worktree。';
  } else if (!issue.control.candidateSha) {
    element('next-action-title').textContent = '验证并冻结候选 SHA';
    element('next-action-detail').textContent = '开发输出不是证据；检查 Git 和测试后记录 candidate SHA。';
  } else if (!review) {
    element('next-action-title').textContent = '开始独立 Review';
    element('next-action-detail').textContent = '选择引擎，新会话只审冻结 SHA，不继承开发者主观上下文。';
  } else {
    element('next-action-title').textContent = issue.nextAction.label;
    element('next-action-detail').textContent = issue.nextAction.detail;
  }
  element('copy-command').classList.add('is-hidden');
  element('discussion-tab-meta').textContent = `${collaboration?.conversations.filter((item) => item.role === 'DISCUSSION').length ?? 0} 个对话`;
  ...
  const generationIsNewerThanFrozen = Boolean(generatedDevelopmentBrief && developmentBriefTurn?.completedAt && (!brief || developmentBriefTurn.completedAt > brief.frozenAt));
  if (generatedDevelopmentBrief && developmentBriefTurn && generationIsNewerThanFrozen && appliedDevelopmentBriefTurnIds.get(issue.number) !== developmentBriefTurn.id) {
  ...
  generateBrief.disabled = deliveryBlocked || !discussion || discussion.messages.length === 0 || discussion.status === 'RUNNING';
  generateBrief.textContent = developmentBriefTurn?.status === 'RUNNING' ? '正在生成开发说明…' : '根据讨论生成开发说明';
  element<HTMLButtonElement>('freeze-brief').disabled = deliveryBlocked || !discussion || !draft.value.trim();
  element<HTMLButtonElement>('freeze-brief').textContent = brief ? '确认为新版本' : '确认开发说明';
  element('brief-message').textContent = deliveryBlocked ? `等待 ${blockerText} 关闭后才能生成、编辑或确认开发说明。` : '';

  element('workspace-binding').textContent = workspace?.branch ?? '尚未创建';
  element('workspace-binding-meta').textContent = workspace ? `${workspace.path} · base ${shortSha(workspace.baseSha)}` : brief ? '已具备创建条件' : '确认开发说明后才能创建';
  ...
  element('developer-status').textContent = implementation?.status ?? (developerReady ? 'READY' : 'LOCKED');
  element('developer-engine').textContent = implementation ? `${implementation.engine.toUpperCase()} · ${roleLabels[implementation.role]}` : '选择引擎';
  ...
  const reviewerReady = Boolean(brief && workspace && issue.control.candidateSha && !paused && !deliveryBlocked);
  ...
  element('reviewer-engine').textContent = review ? `${review.engine.toUpperCase()} · REVIEW` : '选择引擎';
  ...
  element('form-message').textContent = deliveryBlocked ? `等待 ${blockerText} 关闭后才能修改交付状态。` : '';
...
function updateCandidateButton() {
  ...
  element<HTMLButtonElement>('freeze-candidate').disabled = !/^[0-9a-f]{7,40}$/i.test(value) || Boolean(issue?.attention.required) || !dependenciesReady;
}

function renderConversationTabs() {
  const conversations = collaboration?.conversations ?? [];
  const tabs = element('conversation-tabs');
  tabs.innerHTML = conversations.length ? conversations.map((conversation) => `
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-conversation]').forEach((button) => button.addEventListener('click', () => {
...
function renderChat() {
  ...
  if (!conversation) {
    ...
    element('chat-messages').innerHTML = '<p class="empty">点击“新建”，选择 Claude Code 或 Codex 开始讨论。</p>';
  ...
  element('active-conversation-title').textContent = conversation.engine === 'claude' ? 'Claude Code' : 'Codex';
  const sessionLabel = conversation.sessionId ? `SESSION ${conversation.sessionId}` : 'SESSION PENDING';
  ...
  const messageMarkup = conversation.messages.length ? conversation.messages.map((message) => {
    ...
    const author = generatedPrompt ? 'QIANSHOU' : message.role === 'USER' ? 'YOU' : message.role === 'AGENT' ? conversation.engine.toUpperCase() : 'SYSTEM';
    const body = message.role === 'AGENT'
  ...
  const eventTimeline = runEvents.length ? `<ol class="agent-event-list">${runEvents.map((event) => `
    ...
    </li>`).join('')}</ol>` : '<p class="agent-event-empty">等待 Agent 输出第一个事件……</p>';
  const runSummary = latestTurn?.progress
    ...
    : '';
  const progressMarkup = latestTurn?.progress ? (runningTurn ? `
...
function renderStopConditions() {
  ...
  list.innerHTML = conditions.length ? conditions.map((stop) => `
    ...
    </article>`).join('') : '<p class="empty stop-empty">当前没有 Stop Condition。开发或 Review 遇到需要判断的问题时，从这里带着恢复点回流讨论。</p>';
  document.querySelectorAll<HTMLButtonElement>('[data-resolve-stop]').forEach((button) => button.addEventListener('click', () => {
    activeStopConditionId = button.dataset.resolveStop ?? null;
    const stop = conditions.find((item) => item.id === activeStopConditionId);
    element('stop-resolution-title').textContent = stop ? `解决：${stop.summary}` : '解决 Stop Condition';
...
function renderActivity() {
  ...
  element('checkpoint-stops').textContent = String(issue.attention.openStopConditionCount).padStart(2, '0');
  element('checkpoint-conversations').textContent = String(collaboration?.conversations.length ?? 0).padStart(2, '0');
  ...
  for (const stop of collaboration?.stopConditions ?? []) {
    activities.push({
      ...
      detail: stop.status === 'OPEN' ? `保留在 ${PHASE_LABELS[stop.originPhase]}` : stopOutcomeLabels[stop.outcome ?? 'CONTINUE'],
      tone: stop.status === 'OPEN' ? 'is-attention' : 'is-success'
    });
  }
  for (const item of collaboration?.briefs ?? []) {
    activities.push({ at: item.frozenAt, label: `确认开发说明 v${item.version}`, detail: `来自 Discussion ${item.sourceConversationId.slice(0, 8)}`, tone: 'is-success' });
  }
  for (const conversation of collaboration?.conversations ?? []) {
    activities.push({ at: conversation.createdAt, label: `新建${roleLabels[conversation.role]}对话`, detail: `${conversation.engine === 'claude' ? 'Claude Code' : 'Codex'} · ${conversation.status}` });
  }
  for (const event of requireSnapshot().events.filter((item) => item.issueNumber === issue.number)) {
    activities.push({ at: event.at, label: `交付状态：${PHASE_LABELS[event.after.phase]}`, detail: event.note || `Candidate ${shortSha(event.after.candidateSha)}` });
  }
  activities.sort((left, right) => right.at.localeCompare(left.at));
  element('activity-list').innerHTML = activities.length ? activities.slice(0, 30).map((activity) => `
    <li class="${activity.tone ?? ''}"><strong>${escapeHtml(activity.label)}</strong><span>${escapeHtml(activity.detail)}</span><small>${formatTime(activity.at)}</small></li>`).join('') : '<li class="empty">尚无本地协作记录</li>';
}
...
function openConversationForm(role: ConversationRole = 'DISCUSSION') {
  ...
  if (role !== 'DISCUSSION' && !dependencies.ready) {
    const blockers = dependencies.blockers.map((number) => `#${number}`).join('、') || 'GitHub 状态未知';
...
async function createConversation(event: SubmitEvent) {
  ...
  try {
    ...
    showToast(`${roleLabels[role]}对话已创建，引擎固定为 ${engine === 'claude' ? 'Claude Code' : 'Codex'}`);
  } catch (error: unknown) {
...
async function cancelActiveTurn() {
  const conversation = activeConversation();
  const turn = collaboration?.turns.find((item) => item.conversationId === conversation?.id && item.status === 'RUNNING');
  if (!turn || !window.confirm('确定停止本次 Agent 运行吗？已经产生的对话和运行记录会保留。')) return;
  const cancel = element<HTMLButtonElement>('cancel-turn');
...
async function generateDevelopmentBrief() {
  ...
  message.textContent = '';
  try {
    await api(`/api/conversations/${encodeURIComponent(discussion.id)}/development-brief`, { method: 'POST' });
    await refreshCollaboration();
...
async function freezeCandidate(event: SubmitEvent) {
  ...
  try {
    await api(`/api/issues/${issue.number}/control`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ phase: 'CANDIDATE_READY', candidateSha, note: 'Candidate SHA frozen after human evidence check' })
    });
    await refresh(true);
...
async function refreshCollaboration() {
  ...
  try {
    ...
    collaboration = next;
    if (!activeConversationId || !next.conversations.some((item) => item.id === activeConversationId)) {
...
async function refresh(force = false) {
  ...
  try {
    ...
    if (!snapshot.issues.some((issue) => issue.number === selectedIssueNumber)) selectedIssueNumber = snapshot.issues[0]?.number ?? 0;
    renderOverview();
    renderIssueList();
    renderSelectedIssue();
    await refreshCollaboration();
...
document.querySelectorAll<HTMLButtonElement>('[data-workbench-view]').forEach((button) => button.addEventListener('click', () => {
  setWorkbenchView(button.dataset.workbenchView === 'delivery' ? 'delivery' : 'discussion');
}));
element<HTMLButtonElement>('new-conversation').addEventListener('click', () => openConversationForm('DISCUSSION'));
element<HTMLButtonElement>('cancel-conversation').addEventListener('click', () => element('conversation-create-form').classList.add('is-hidden'));
element<HTMLFormElement>('conversation-create-form').addEventListener('submit', (event) => void createConversation(event));
element<HTMLFormElement>('chat-form').addEventListener('submit', (event) => void sendMessage(event));
element<HTMLButtonElement>('cancel-turn').addEventListener('click', () => void cancelActiveTurn());
element<HTMLButtonElement>('generate-development-brief').addEventListener('click', () => void generateDevelopmentBrief());
element<HTMLButtonElement>('freeze-brief').addEventListener('click', () => void freezeBrief());
element<HTMLTextAreaElement>('brief-draft').addEventListener('input', (event) => briefDrafts.set(selectedIssueNumber, (event.target as HTMLTextAreaElement).value));
element<HTMLButtonElement>('create-workspace').addEventListener('click', () => void createWorkspace());
element<HTMLButtonElement>('start-implementation').addEventListener('click', () => openConversationForm('IMPLEMENTATION'));
element<HTMLButtonElement>('start-review').addEventListener('click', () => openConversationForm('REVIEW'));
element<HTMLFormElement>('candidate-form').addEventListener('submit', (event) => void freezeCandidate(event));
element<HTMLInputElement>('candidate-input').addEventListener('input', updateCandidateButton);
element<HTMLFormElement>('control-form').addEventListener('submit', (event) => void saveControl(event));
...
element<HTMLButtonElement>('cancel-stop-condition').addEventListener('click', () => element<HTMLFormElement>('stop-condition-form').classList.add('is-hidden'));
element<HTMLFormElement>('stop-condition-form').addEventListener('submit', (event) => void createStopCondition(event));
element<HTMLButtonElement>('cancel-stop-resolution').addEventListener('click', () => {
  activeStopConditionId = null;
  element<HTMLFormElement>('stop-resolution-form').classList.add('is-hidden');
});
element<HTMLFormElement>('stop-resolution-form').addEventListener('submit', (event) => void resolveStopCondition(event));
