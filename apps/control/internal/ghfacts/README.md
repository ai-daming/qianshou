...
[`obligations.json`](obligations.json). The binding is mechanical:
`go run ./internal/obligations/cmd/check` (CI entry) validates the
manifest's schema and status enum, resolves every citation in its named
package, requires README↔manifest ID agreement in both directions,
forces every listed test to be classified (cited, or declared uncited
with a reason), and runs all cited tests. The checker itself carries
tamper tests (`internal/obligations/obligations_test.go`).

| Layer | 良构 | 绑定 | 歧义 | 信道 | 新鲜度 |
|---|---|---|---|---|---|
| 传输 transport | ENFORCED: GH-T-WF-LIMIT, GH-T-WF-BODY, GH-T-WF-STATUS | N/A — transport carries no identity; binding lives where identity fields decode (schema) | ENFORCED: GH-T-AM-DUPKEYS, GH-T-AM-UTF8 | ENFORCED: GH-T-CH-DEADLINE, GH-T-CH-MEDIATYPE, GH-T-CH-REDIRECT; UNVERIFIED: GH-ENV-PROXY, GH-ENV-TLS | N/A — single responses are atomic; multi-response conflicts belong to aggregation |
| 协议 protocol | ENFORCED: GH-P-WF-PAGEINFO, GH-P-WF-TERMINATION | ENFORCED: GH-P-BD-NEXTURL | ENFORCED: GH-P-AM-GRAMMAR (single owner parseLinkHeader with the full ABNF alphabet: tchar, qdtext, quoted-pair, relation-type per RFC 9110 §5.6/8288 §3) | ENFORCED: GH-P-CH-ORIGIN | ACCEPTED RESIDUAL: GH-A-FR-ATOMICITY |
...
| 语义 semantic | ENFORCED: GH-SE-WF-INVARIANTS (unified Validate on opaque types) | ENFORCED: GH-SE-BD-SELFREF | ENFORCED (duplicate labels/blockers fail, unified invariants) | N/A | N/A |
...
| 消费 consumption | ENFORCED: GH-C-WF-UNFORGEABLE (opaque facts; the endpoint-override constructor exists only inside test compilation via export_test.go; production code cannot redirect the fact source), GH-C-WF-MULTICONTROL | ENFORCED: GH-C-BD-FETCHFAIL | N/A | N/A | N/A |
...
| Public endpoint-override constructor | A self-controlled server self-reports every binding field and mints facts | constructor exists only in test compilation (export_test.go → NewClientForTest, `GH-C-WF-UNFORGEABLE`) |
| Link grammar without alphabet | Structural parsing accepts illegal characters (quotes in tokens, invalid relation-types, non-tchar names) | full ABNF transcription in parseLinkHeader (`GH-P-AM-GRAMMAR`) |
| Shell checker without schema | Typo'd statuses, wrong-package citations, unclassified tests pass | Go checker with tamper tests (`internal/obligations`) |
