...
// sources. It reads Git facts; it never writes them.
func VerifyGitBinding(path, slug string) error {
	...
	remoteSlug, ok := slugFromRemote(remote)
	if !ok {
		return fmt.Errorf("无法从 origin remote %q 解析 owner/repo", remote)
	}
...
func slugFromRemote(remote string) (string, bool) {
